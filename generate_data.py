...
def split_dataset(dev_data_cnt=5000):
    for e in ['TNEWS', 'OCNLI', 'OCEMOTION']:
        cnt = 0
        with open('./tianchi_datasets/' + e + '/total.csv', encoding='UTF-8') as f:
            with open('./tianchi_datasets/' + e + '/train.csv', 'w', encoding='UTF-8') as f_train:
                with open('./tianchi_datasets/' + e + '/dev.csv', 'w', encoding='UTF-8') as f_dev:
                    for line in f:
                        cnt += 1
                        if cnt <= dev_data_cnt:
                            f_dev.write(line)
